...
count_rows <- function(subset, n, resp_name, select_from = F, categories, year = 2022){
  ...
  if (year == 2022){
    ...
    result <- rbind.data.frame(result, full_cities21 %>% filter(!org %in% unique(result$org) & !org %in% na_set$org) %>%
                                 select(id, org) %>%
...
sum_for_multiple_responses <- function(subset, n, resp_name, year = 2022){
...
mean_for_multiple_responses <- function(subset, n, resp_name, year = 2022){
  result <- subset %>% group_by(id, org) %>% filter(coln == n) %>% 
    filter(!resp == 9999999 & !resp == "") %>%
...
max_for_multiple_responses <- function(subset, n, resp_name, year = 2022){
  result <- subset %>% group_by(id, org) %>% filter(coln == n) %>% 
    filter(!resp == 9999999 & !resp == "") %>%
